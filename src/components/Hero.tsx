...
export default function Hero() {
  ...
  // Get all available properties for rotation
  const allProperties = [
    ...luxuryVillas,
    ...luxuryPenthouses,
  ];
  ...
  const PROPERTIES_PER_VIEW = 5;
  const ROTATION_INTERVAL = 20000; // 20 seconds

  // Auto-rotate properties every 20 seconds
  useEffect(() => {
    const interval = setInterval(() => {
      setIsTransitioning(true);
      setTimeout(() => {
        setCurrentIndex((prevIndex) => {
          const nextIndex = prevIndex + PROPERTIES_PER_VIEW;
          // Loop back to start if we've shown all properties
          return nextIndex >= allProperties.length ? 0 : nextIndex;
        });
  ...
  // Get current set of properties to display
  const featuredProducts = allProperties.slice(
    currentIndex,
    currentIndex + PROPERTIES_PER_VIEW
  ).concat(
    // If we don't have enough properties, loop from the beginning
    currentIndex + PROPERTIES_PER_VIEW > allProperties.length
      ? allProperties.slice(0, (currentIndex + PROPERTIES_PER_VIEW) - allProperties.length)
      : []
  ).slice(0, PROPERTIES_PER_VIEW);
    ...
    <section id="home" className="relative min-h-screen flex flex-col justify-center pt-32 pb-12 overflow-hidden">
      ...
      <div className="relative z-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full flex flex-col items-center">
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-6 mt-24 mb-12">
          ...
          {/* Product Cards Grid with Auto-Rotation */}
          <div className="relative">
            {/* Carousel Indicator */}
            <div className="absolute -top-8 right-0 flex items-center gap-2 text-xs font-bold text-gray-600 dark:text-gray-400">
              <div className="flex gap-1">
                {Array.from({ length: Math.ceil(allProperties.length / PROPERTIES_PER_VIEW) }).map((_, index) => (
                  <div
                    key={index}
                    className={`h-1.5 rounded-full transition-all duration-500 ${
                      Math.floor(currentIndex / PROPERTIES_PER_VIEW) === index
              ...
              <span className="text-cyan-600 dark:text-cyan-400">
                {Math.floor(currentIndex / PROPERTIES_PER_VIEW) + 1}/{Math.ceil(allProperties.length / PROPERTIES_PER_VIEW)}
              </span>
            </div>
            ...
            <motion.div
              ...
              animate={{ opacity: isTransitioning ? 0 : 1, x: isTransitioning ? -100 : 0 }}
            ...
            >
              {featuredProducts.map((product, idx) => (
                <motion.div
                  ...
                  onClick={() => navigate(`/properties/${product.id}`)}
